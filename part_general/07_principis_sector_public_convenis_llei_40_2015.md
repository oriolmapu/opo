...
```mermaid
graph TD
    SectorPublic["SECTOR PÚBLIC (Art. 2 Llei 40/2015)"]
    
    subgraph AP["ADMINISTRACIONS PÚBLIQUES (Art. 2.3)"]
        AGE["Administració General de l'Estat (AGE)"]
        ACCA["Administracions de les Comunitats Autònomes"]
        AEL["Entitats que integren l'Administració Local"]
        SPI_AP["Organismes públics i entitats de dret públic vinculades o dependents"]
    end
    
    subgraph SPI["SECTOR PÚBLIC INSTITUCIONAL"]
        OA["Organismes autònoms"]
        EPEL["Entitats públiques empresarials"]
        SM["Societats mercantils públiques (>50% capital públic)"]
        Fundacions["Fundacions del sector públic"]
        Consorcis["Consorcis"]
        Univ["Universitats públiques (per la seva legislació específica)"]
    end
...
graph TD
    ...
    Encomana["ENCOMANA DE GESTIÓ (Art. 11)<br/>Traspàs d'activitats de caràcter MATERIAL o TÈCNIC.<br/>NO cedeix la titularitat ni la competència."]
...
```mermaid
graph LR
    Garantia["GARANTIA D'IMPARCIALITAT"]
    
    Abs["ABSTENCIÓ (Art. 23)<br/>Iniciativa de la PRÒPIA AUTORITAT o funcionari.<br/>Ha de comunicar-ho al seu superior immediat.<br/>La no abstenció genera responsabilitat disciplinària."]
    Rec["RECUSACIÓ (Art. 24)<br/>Iniciativa dels INTERESSATS en qualsevol moment.<br/>Es formula per escrit justificant la causa.<br/>El superior resol en 3 dies sense que càpiga recurs."]
...
```mermaid
graph TD
    Vigencia["DURADA DELS CONVENIS ADMINISTRATIUS (Art. 49.h)"]
    
    TerminiInicial["TERMINI INICIAL MÀXIM: 4 ANYS<br/>(Llevat que una llei especial n'estableixi un de superior)"]
    Prorroga["PRÒRROGA EXPRESSA: FINS A 4 ANYS ADDICIONALS<br/>- Requereix acord unànime de les parts<br/>- S'ha de formalitzar ABANS que finalitzi el termini inicial"]
    LimitTotal["DURADA MÀXIMA TOTAL ORDINÀRIA: 8 ANYS (4 + 4)"]

    Vigencia --> TerminiInicial
    TerminiInicial --> Prorroga
    Prorroga --> LimitTotal
```
