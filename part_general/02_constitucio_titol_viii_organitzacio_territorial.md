...
```mermaid
graph TD
    TitolVIII["TÍTOL VIII DE LA CONSTITUCIÓ ESPANYOLA<br/>(Articles 137 - 158)"]
    
    Cap1["Capítol I: Principis generals<br/>(Arts. 137 - 139)"]
    Cap2["Capítol II: De l'Administració local<br/>(Arts. 140 - 142)"]
    Cap3["Capítol III: De les Comunitats Autònomes<br/>(Arts. 143 - 158)"]
    
    TitolVIII --> Cap1
    TitolVIII --> Cap2
    TitolVIII --> Cap3
    
    Cap1 --> P1["Art. 137: Divisió territorial (Municipis, Províncies, CCAA)"]
    Cap1 --> P2["Art. 138: Solidaritat, equilibri econòmic i insularitat"]
    Cap1 --> P3["Art. 139: Igualtat de drets i llibertat de circulació"]
    
    Cap2 --> L1["Art. 140: Autonomia municipal i Ajuntaments"]
    Cap2 --> L2["Art. 141: Província, Diputacions i règim insular"]
    Cap2 --> L3["Art. 142: Suficiència de les Hisendes Locals"]
    
    Cap3 --> C1["Arts. 143-147: Vies d'accés i Estatuts d'Autonomia"]
    Cap3 --> C2["Arts. 148-150: Sistema de distribució de competències"]
    Cap3 --> C3["Arts. 151-152: Procediment especial i organització institucional"]
    Cap3 --> C4["Arts. 153-155: Control de les CCAA i mecanisme de l'Art. 155"]
    Cap3 --> C5["Arts. 156-158: Autonomia financera i Fons de Compensació"]
```
...
| :--- | :--- | :--- |
| **Iniciativa autonòmica** | - Totes les **Diputacions** provincials interessades (o òrgan interinsular).<br/>- **2/3 dels municipis** la població dels quals representi la majoria del cens electoral de cada província o illa. | - Totes les **Diputacions** (o òrgan interinsular).<br/>- **3/4 dels municipis** que representin la majoria del cens electoral.<br/>- **Referèndum d'iniciativa** aprovat per majoria absoluta del cens de cada província. |
...
| **Aprovació definitiva** | Tramitació de llei orgànica a les Corts Generals (sense referèndum ciutadà d'aprovació). | 1. Examen conjunt amb la Comissió Constitucional del Congrés (2 mesos).<br/>2. **Referèndum del cos electoral** aprovat per majoria de vots vàlids.<br/>3. **Ratificació per les Corts Generals**.<br/>4. Sanció i promulgació pel Rei. |
...
```mermaid
graph TD
    Estatut["CONTINGUT MÍNIM DE L'ESTATUT (Art. 147.2 CE)"]
    A["a) Denominació de la Comunitat (d'acord amb la seva identitat històrica)"]
    B["b) Delimitació del seu territori"]
    C["c) Denominació, organització i seu de les institucions autònomes pròpies"]
    D["d) Competències assumides i bases per al traspàs de serveis"]
...
```mermaid
graph LR
    Lleis["LLEIS DE L'ARTICLE 150 CE"]
    L150_1["Art. 150.1: Lleis Marc<br/>(Corts atribueixen potestat legislativa<br/>autonòmica en marc de principis estatals)"]
    L150_2["Art. 150.2: Lleis Orgàniques de Transferència o Delegació<br/>(Llei Orgànica transfereix facultats estatals<br/>amb recursos i controls)"]
    L150_3["Art. 150.3: Lleis d'Harmonització<br/>(Llei ordinària per raó d'interès general;<br/>Apreciació prèvia per majoria absoluta de cada Cambra)"]
...
sequenceDiagram
    autonumber
    participant CA as Comunitat Autònoma
    participant Gov as Govern d'Espanya
    participant Senat as Senat (Majoria Absoluta)
    
    Note over CA: Incompliment flagrant d'obligacions legals/CE<br/>o atemptat greu a l'interès general
    Gov->>CA: 1. REQUERIMENT PREVI al President de la CA
    alt El requeriment és atès
        CA-->>Gov: Es restableix la legalitat (finalitza el procés)
    else El requeriment NO és atès
        Gov->>Senat: 2. Sol·licitud d'autorització de mesures
        Senat-->>Gov: 3. Aprovació per MAJORIA ABSOLUTA del Senat
        Gov->>CA: 4. Execució de mesures d'obligació forçosa<br/>(instruccions directes a totes les autoritats autonòmiques)
    end
```
